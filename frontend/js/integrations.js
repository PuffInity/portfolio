/* ==========================================
 * Integrations service page renderer
 * ========================================== */
...
function renderHero(hero) {
    ...
    if (isStr(hero.badge)) {
        const badgeText = $('#hero-badge [data-bind="badge"]');
    ...
    if (hero.cta1?.text) {
        ...
        if (hero.cta1.href) {
            ...
            if (link) link.href = hero.cta1.href;
        }
    }
    ...
    if (hero.cta2?.text) {
        ...
        if (hero.cta2.href) {
            ...
            if (link) link.href = hero.cta2.href;
        }
    }
...
function renderTerms(t) {
    ...
    if (isStr(t?.quickStart)) {
        const quickText = $('#quickstart-text');
        if (quickText) {
            quickText.textContent = t.quickStart;
            show($('#quickstart'));
        }
    }
...
/**
 * Main page renderer for the integrations service page.
 */
...
// Endpoint for integrations service content.
const INTEGRATIONS_SERVICE_PATH = '/main/integrations';

/**
 * Loads integrations service data and renders the page.
 */
async function loadServicePage() {
    const apiData = await safeFetchJson(INTEGRATIONS_SERVICE_PATH, {
        method: 'GET',
